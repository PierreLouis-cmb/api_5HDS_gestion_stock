...
if($_SERVER['REQUEST_METHOD'] == 'DELETE'){
    ...
    if(isset($_GET['token'])){
        ...
        if(intval($token) > 0){
            ...
            $querySqlVerf = $db->prepare("SELECT token FROM users where token = :token");
            ...
            if($results){
                $querySql = $db->prepare("DELETE FROM users WHERE token = :token");
                $querySql->bindParam(':token',$token);
                $querySql->execute();
                $retour["Success"] = true;
                $retour["Message"] = "L'utilisateur à bien été supprimé"; //Faire une fonction success true

            }else{
                $retour['Success']= false;
                $retour['Message'] = "Cette utilisateur n'existe pas";
